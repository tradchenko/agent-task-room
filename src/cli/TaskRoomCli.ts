...
interface LaunchState {
  ...
  localA2AJsonRpcUrl: string;
  publicA2ARestUrl: string;
  localA2ARestUrl: string;
  publicUiUrl: string;
  localUiUrl: string;
  publicRoomUrl: string;
  localRoomUrl: string;
  publicPeerInviteUrl: string;
  localPeerInviteUrl: string;
  hostParticipant: ParticipantIdentity;
  peerParticipant: ParticipantIdentity;
  commands: {
...
export class TaskRoomCli {
  ...
  private readonly runtime: TaskRoomCliRuntime;

  public constructor(private readonly cwd: string = process.cwd(), runtime?: TaskRoomCliRuntime) {
    this.statePaths = new StatePaths(this.cwd);
    this.runtime = runtime ?? new DefaultTaskRoomCliRuntime();
  }

  public async run(command: string, options: CliOptions): Promise<void> {
    switch (command) {
      case 'help':
      case '--help':
      case '-h':
  ...
  private printBanner(title: string): void {
...
`);
  ...
  private readLocalState(): LocalSessionState | null {
    return readJsonIfExists<LocalSessionState>(this.statePaths.sessionFile);
  }

  private writeLocalState(state: LocalSessionState): void {
    writeJson(this.statePaths.sessionFile, state);
  }

  private resolveAuthToken(options: CliOptions, localState: LocalSessionState | null): string | null {
    return (
  ...
  private resolveServerUrl(options: CliOptions, localState: LocalSessionState | null): string {
    return (
  ...
  private resolveRoomId(options: CliOptions, localState: LocalSessionState | null): string | null {
    return (
      getStringOption(options, 'room-id') ??
  ...
  private resolveParticipant(options: CliOptions, localState: LocalSessionState | null): ParticipantIdentity {
    const localParticipant = localState?.participant ?? null;
    const participantId =
      getStringOption(options, 'participant-id') ??
    ...
    const sameParticipant =
      localParticipant && localParticipant.participantId === participantId ? localParticipant : null;

    return {
      participantId,
      participantLabel:
        getStringOption(options, 'participant-label') ??
        process.env.AGENT_TASK_ROOM_PARTICIPANT_LABEL ??
        sameParticipant?.participantLabel ??
        participantId,
      role:
        getStringOption(options, 'role') ??
  ...
  private readToolStructuredContent<T extends Record<string, unknown>>(
    toolName: string,
  ...
  private formatMessage(message: RoomUpdateMessage): string {
  ...
  private findParticipant(overview: RoomOverview, participantId: string): RoomOverview['participants'][number] | null {
    return overview.participants.find((participant) => participant.participantId === participantId) ?? null;
  }

  private buildNextSteps(overview: RoomOverview, participant: ParticipantIdentity): string[] {
    ...
    if (overview.status === 'NEEDS_HUMAN_CONFIRMATION') {
      const allAgree = finalParticipants.every((entry) =>
        ['agree', 'agree_with_risks'].includes(entry.finalStance ?? ''),
      );
  ...
  private writeAlertFiles(payload: {
  ...
  private writePromptArtifacts(payload: {
    joinPosixCommand: string;
    joinPowerShellCommand: string;
    ownerRoomUrl: string;
  ...
  private buildA2AExamples(roomId: string): A2AExamples {
    const watchEnvelope = {
      message: {
  ...
  private buildParticipantRoomUrl(payload: {
    uiUrl: string;
    roomId: string;
    token: string;
    participant: ParticipantIdentity;
  ...
  private buildChatPrompt(roleLabel: string): string {
    return [
  ...
  private resolveWatchInterval(options: CliOptions): number {
  ...
  private async commandOpen(options: CliOptions): Promise<void> {
    ...
    await this.withClient(serverUrl, authToken, async (client) => {
      ...
      const roomData = this.readToolStructuredContent<{
        roomId: string;
        status: string;
        sharedPrompt: string;
      }>('open_task_room', opened);
  ...
  private async commandJoin(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
    const serverUrl = this.resolveServerUrl(options, localState);
    const authToken = this.resolveAuthToken(options, localState);
    const participant = this.resolveParticipant(options, localState);
  ...
  private async commandSync(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
    ...
    await this.withClient(serverUrl, authToken, async (client) => {
      ...
      const overview = this.readToolStructuredContent<{ room: RoomOverview }>(
        'get_room_overview',
        overviewResult,
      ).room;
      const updates = this.readToolStructuredContent<{
        latestSequence: number;
        updates: RoomUpdateMessage[];
      }>('get_room_updates', updatesResult);
  ...
  private async commandWatch(options: CliOptions): Promise<void> {
  ...
  private async commandContext(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async commandMessage(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async commandSearch(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
    const serverUrl = this.resolveServerUrl(options, localState);
    const authToken = this.resolveAuthToken(options, localState);
    ...
    await this.withClient(serverUrl, authToken, async (client) => {
      ...
      const payload = this.readToolStructuredContent<{
        rooms: RoomSearchResult[];
      }>('search_task_rooms', result);
  ...
  private async commandHistory(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
    ...
    await this.withClient(serverUrl, authToken, async (client) => {
      ...
      for (const entry of payload.entries) {
        process.stdout.write(
          `[${entry.createdAt}] ${entry.entryType}/${entry.kind} · ${entry.authorLabel}\n`,
        );
  ...
  private async commandArtifact(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async commandDecision(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async commandPosition(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async commandHumanConfirm(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async commandStatus(options: CliOptions): Promise<void> {
    await this.commandSync({ ...options, once: true });
  }

  private async commandClose(options: CliOptions): Promise<void> {
    const localState = this.readLocalState();
  ...
  private async ensureServer(host: string, port: number, mcpPath: string, token: string, cwd: string): Promise<{
    pid: number;
    baseUrl: string;
    mcpUrl: string;
  }> {
  ...
  private async ensureNgrok(binary: string, localPort: number): Promise<{ pid: number; publicUrl: string }> {
  ...
  private async ensureWatch(intervalSeconds: number): Promise<number> {
  ...
  private async runStartFlow(
    options: CliOptions,
    transportMode: 'public' | 'local',
    commandLabel: 'start' | 'start-local' | 'session' = transportMode === 'local' ? 'start-local' : 'start',
  ): Promise<void> {
    ...
    const existingLaunch = readJsonIfExists<LaunchState>(this.statePaths.launchFile);
  ...
  private async commandStart(options: CliOptions): Promise<void> {
    await this.runStartFlow(options, 'public', 'start');
  }

  private async commandStartLocal(options: CliOptions): Promise<void> {
    await this.runStartFlow(options, 'local', 'start-local');
  }

  private async commandSession(options: CliOptions): Promise<void> {
    await this.runStartFlow(options, 'local', 'session');
  }

  private async commandStop(): Promise<void> {
  ...
  private async commandDoctor(): Promise<void> {
