...
export function inferStorageBackend(storageFile: string, explicitBackend?: StorageBackend): StorageBackend {
  if (explicitBackend) {
...
export async function createRoomRepository(
  options: RoomRepositoryFactoryOptions,
): Promise<CreatedRoomRepository> {
  ...
  const legacyJsonFile =
    options.legacyJsonFile ?? path.join(path.dirname(options.storageFile), 'rooms.json');
