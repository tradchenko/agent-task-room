...
import type { DataPart, Message, Part, Task, TaskArtifactUpdateEvent, TaskStatusUpdateEvent } from '@a2a-js/sdk';
...
import {
  ...
  FINAL_STANCES,
  HUMAN_VERDICTS,
  PARTICIPANT_MESSAGE_KINDS,
  ROOM_ARTIFACT_KINDS,
  ROOM_STATUS,
  type RoomSummary,
} from '../../domain/room/room-types.js';
...
type TaskRoomCommand = z.infer<typeof TASK_ROOM_COMMAND_SCHEMA>;
type WatchRoomCommand = z.infer<typeof WATCH_ROOM_COMMAND_SCHEMA>;

interface CommandExecutionResult {
  command: TaskRoomCommand['command'];
  result: Record<string, unknown>;
  summary: string;
}

interface ActiveWatchTask {
  taskId: string;
  roomId: string;
  contextId: string;
  cursor: number;
...
function stringifyCommandHelp(): string {
  return [
...
function toDataPart(data: Record<string, unknown>): DataPart {
  return {
    kind: 'data',
    data,
  };
...
export class TaskRoomA2AExecutor implements AgentExecutor {
  private readonly activeWatchTasks = new Map<string, ActiveWatchTask>();
  ...
  public async execute(requestContext: RequestContext, eventBus: ExecutionEventBus): Promise<void> {
    ...
    try {
      ...
      eventBus.publish(
        this.buildAgentMessage(requestContext, [
          {
            kind: 'text',
  ...
  private async executeCommand(command: TaskRoomCommand): Promise<CommandExecutionResult> {
    switch (command.command) {
      case 'list_rooms': {
        const result = (await this.roomService.listRooms()) as unknown as Record<string, unknown>;
      ...
      case 'get_room_overview': {
        const result = (await this.roomService.getRoomOverview(command.payload.roomId)) as unknown as Record<string, unknown>;

        return {
          command: command.command,
          result,
          summary: `Статус комнаты ${command.payload.roomId}: ${String((result.room as Record<string, unknown>)?.status ?? 'unknown')}.`,
        };
      }
      ...
      case 'search_rooms': {
        const result = (await this.roomService.searchRooms({
          ...
          status: command.payload?.status as 'all' | undefined,
      ...
      case 'open_room': {
        const result = (await this.roomService.openRoom(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'join_room': {
        const result = (await this.roomService.joinRoom(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'declare_context': {
        const result = (await this.roomService.declareContext(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'post_room_message': {
        const result = (await this.roomService.postRoomMessage(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'get_room_updates': {
        const result = (await this.roomService.getRoomUpdates(
          command.payload.roomId,
          command.payload.afterSequence ?? 0,
          command.payload.limit ?? 100,
        )) as unknown as Record<string, unknown>;
      ...
      case 'get_room_history': {
        const result = (await this.roomService.getRoomHistory(
          command.payload.roomId,
          command.payload.limit ?? 50,
        )) as unknown as Record<string, unknown>;
      ...
      case 'submit_final_position': {
        const result = (await this.roomService.submitFinalPosition(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'record_human_feedback': {
        const result = (await this.roomService.recordHumanFeedback(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'add_artifact': {
        const result = (await this.roomService.addArtifact(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'record_decision': {
        const result = (await this.roomService.recordDecision(command.payload)) as unknown as Record<string, unknown>;
      ...
      case 'close_task_room': {
        const result = (await this.roomService.closeRoom(command.payload)) as unknown as Record<string, unknown>;
  ...
  private async executeWatchRoom(
    command: WatchRoomCommand,
    requestContext: RequestContext,
    eventBus: ExecutionEventBus,
  ): Promise<void> {
    const overview = await this.roomService.getRoomOverview(command.payload.roomId);
    const activeWatch: ActiveWatchTask = {
  ...
  private queueWatchFlush(taskId: string): void {
    const activeWatch = this.activeWatchTasks.get(taskId);
  ...
  private async flushWatchTask(taskId: string): Promise<void> {
    const activeWatch = this.activeWatchTasks.get(taskId);
  ...
  private finishWatchTask(
    ...
    activeWatch.eventBus.publish(
      this.buildWatchStatusUpdate(activeWatch.taskId, activeWatch.contextId, state, text),
    );
    activeWatch.eventBus.finished();
  }

  private buildWatchTask(requestContext: RequestContext, room: RoomSummary): Task {
    return {
      kind: 'task',
  ...
  private buildDirectCommandTask(requestContext: RequestContext, summary: string): Task {
    return {
      kind: 'task',
      id: requestContext.taskId,
      contextId: requestContext.contextId,
  ...
  private buildWatchArtifactUpdate(
    activeWatch: ActiveWatchTask,
    room: RoomSummary,
    updates: Awaited<ReturnType<RoomService['getRoomUpdates']>>,
  ...
  private extractRawEnvelope(parts: Part[]): unknown {
    ...
    const textPart = parts.find((part): part is Extract<Part, { kind: 'text' }> => part.kind === 'text');
  ...
  private buildAgentMessage(requestContext: RequestContext, parts: Part[]): Message {
    return {
      kind: 'message',
      messageId: randomUUID(),
      role: 'agent',
