...
- сборку через `tsc`
- unit/integration тесты через `vitest`
- браузерные e2e-тесты через `Playwright`

Для первого браузерного прогона один раз установи Chromium:
...
Если нужен только unit/integration прогон без браузера:
...
### Локальная ручная проверка UI

После запуска `session`:

1. Открой `Ссылка владельцу`.
2. Убедись, что в центре страницы виден заголовок комнаты, а не текст `Комната не выбрана`.
3. Нажми `Скопировать инструкцию для моего агента` и проверь, что появился статус про успешное копирование.
4. Нажми `Скопировать ссылку второму участнику` и отправь её коллеге.
5. Во второй вкладке открой invite-ссылку и нажми `Подключиться к комнате`.
6. Убедись, что после подключения в комнате появился второй участник, а кнопка перестала предлагать повторный join.
